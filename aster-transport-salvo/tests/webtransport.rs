//! Real WebTransport (HTTP/3) round-trip: a `wtransport` client opens a bidi
//! stream, writes an Aster header frame + request, and reads back the response
//! frames + trailer. Proves the QUIC/H3 listener + the WT handler + per-call
//! stream priority end to end. (Aster RPC over WebTransport, browser-shaped.)

use std::time::Duration;

use aster::rpc::codec::{decode_rpc_status, encode_stream_header, SerializationMode, StreamHeader};
use aster::rpc::{Call, RpcStatus, Server, ServiceDispatch};
use aster::{AsterConfig, Node, RelayMode};
use aster_transport_core::framing::{
    decode_frame, encode_frame, FLAG_END_STREAM, FLAG_HEADER, FLAG_TRAILER,
};
use salvo::prelude::*;
use tokio::io::AsyncReadExt;
use wtransport::{ClientConfig, Endpoint};

struct Echo;

#[aster::rpc::async_trait]
impl ServiceDispatch for Echo {
    fn name(&self) -> &str {
        "Echo"
    }
    fn version(&self) -> i32 {
        1
    }
    fn methods(&self) -> &[&str] {
        &["unary"]
    }
    async fn dispatch(&self, _method: &str, mut call: Call) {
        let req = call.recv_request().await.unwrap_or_default();
        let resp: Vec<u8> = req.into_iter().rev().collect();
        let _ = call.respond(resp, &RpcStatus::ok());
    }
}

fn header_with_priority() -> StreamHeader {
    StreamHeader {
        service: "Echo".into(),
        method: "unary".into(),
        version: 1,
        call_id: 0,
        deadline: 0,
        serialization_mode: SerializationMode::Xlang.as_i8(),
        // Exercises the per-call stream-priority path (urgency 0 = most urgent).
        metadata_keys: vec!["aster-priority".into()],
        metadata_values: vec!["0".into()],
        session_id: 0,
    }
}

#[tokio::test]
async fn webtransport_unary_roundtrip() {
    // Several rustls stacks linked in this test binary — install a provider.
    let _ = rustls::crypto::aws_lc_rs::default_provider().install_default();

    let cfg = AsterConfig::builder()
        .relay(RelayMode::Disabled)
        .bind_addr("127.0.0.1:0")
        .build();
    let node = Node::start(cfg).await.unwrap();
    let dispatcher = Server::new(&node).register(Echo).dispatcher();
    let service = Service::new(aster_transport_salvo::wt_router(dispatcher));

    let port = {
        let l = std::net::UdpSocket::bind("127.0.0.1:0").unwrap();
        l.local_addr().unwrap().port()
    };
    let addr = format!("127.0.0.1:{port}");
    let tls = aster_transport_salvo::TlsMaterial::self_signed(["localhost".into()]);
    tokio::spawn(async move {
        let _ = aster_transport_salvo::serve_https(&addr, tls, service).await;
    });

    // WebTransport client (accept the self-signed cert).
    let client = Endpoint::client(
        ClientConfig::builder()
            .with_bind_default()
            .with_no_cert_validation()
            .build(),
    )
    .unwrap();

    let url = format!("https://127.0.0.1:{port}/aster/wt");
    let mut conn = None;
    for _ in 0..40 {
        match client.connect(&url).await {
            Ok(c) => {
                conn = Some(c);
                break;
            }
            Err(_) => tokio::time::sleep(Duration::from_millis(50)).await,
        }
    }
    let conn = conn.expect("WebTransport server did not come up");

    // One bidi stream = one call: header frame, then the request frame.
    let (mut send, mut recv) = conn.open_bi().await.unwrap().await.unwrap();
    let hdr = encode_stream_header(&header_with_priority()).unwrap();
    send.write_all(&encode_frame(&hdr, FLAG_HEADER).unwrap())
        .await
        .unwrap();
    send.write_all(&encode_frame(&[1, 2, 3], FLAG_END_STREAM).unwrap())
        .await
        .unwrap();

    // Read the full response (server shuts its send half after the trailer).
    let mut bytes = Vec::new();
    recv.read_to_end(&mut bytes).await.unwrap();

    let (payload, _f, consumed) = decode_frame(&bytes).unwrap();
    assert_eq!(payload, vec![3, 2, 1]);
    let (trailer, tflags, _) = decode_frame(&bytes[consumed..]).unwrap();
    assert!(tflags & FLAG_TRAILER != 0);
    assert_eq!(decode_rpc_status(&trailer).unwrap().code, 0);

    node.shutdown().await;
}
