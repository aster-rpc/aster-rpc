...
|---|---|---|---|
| `root_pubkey` | `bytes \| None` | Resolved from config | 32-byte ed25519 root public key (deployment trust anchor) |
...
When no `config=` is passed, `AsterServer` calls `AsterConfig.from_env()`. Common environment variables:

```bash
# Trust
ASTER_ROOT_PUBKEY_FILE=~/.aster/root_pub.key    # root public key (hex or JSON with public_key field)
ASTER_ALLOW_ALL_CONSUMERS=false           # default: false
ASTER_ALLOW_ALL_PRODUCERS=true            # default: true

# Network
ASTER_RELAY_MODE=default
ASTER_SECRET_KEY=<base64-32-bytes>        # stable node identity across restarts
...
```python
config = AsterConfig.from_file("aster.toml")
async with AsterServer(services=[...], config=config) as srv:
    ...
```
...
```
  [trust]
    root_pubkey                 : a1b2c3d4...                        (ASTER_ROOT_PUBKEY_FILE)
    allow_all_consumers         : False                              (default)
    allow_all_producers         : True                               (default)
  [network]
    secret_key                  : ****...deadbeef                    (ASTER_SECRET_KEY)
