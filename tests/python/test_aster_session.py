...
@fory_tag("test.session/Req")
@dataclass
class Req:
    value: int = 0


@fory_tag("test.session/Resp")
@dataclass
class Resp:
    result: int = 0
...
async def test_local_session_unary_no_trailer():
    ...
    call_header = CallHeader(method="add", call_id="call-001")
...
async def test_local_session_unary_error_trailer_only():
    ...
    call_header = CallHeader(method="fail", call_id="call-err")
    await write_frame(c_send, codec.encode(call_header), flags=CALL)
...
async def test_local_session_cancel_mid_call():
    ...
    # Start the slow call
    call_header = CallHeader(method="slow", call_id="call-slow")
...
async def test_local_session_mid_call_call_rejection():
    ...
    await write_frame(c_send, b_codec.encode(CallHeader(method="block", call_id="c1")), flags=CALL)
    ...
    await write_frame(c_send, b_codec.encode(CallHeader(method="block", call_id="c2")), flags=CALL)
...
async def test_stream_discriminator_mismatch_shared():
    ...
    @fory_tag("test.session/EchoReq")
    @dataclass
    class EchoReq:
        msg: str = ""

    @fory_tag("test.session/EchoResp")
    @dataclass
    class EchoResp:
        msg: str = ""
